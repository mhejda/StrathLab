...
for file = files'
    ...
    if contains(file.name, 'Wfm') ~= 1;
        ...
        for i=1:dim(2)
            ...
            fname  = append(file.name(1:end-4),'_{wf',num2str(i),'}_{Y}.csv');
            writematrix(I(:,i),fname); 
            fnameX = append(file.name(1:end-4),'_{wf',num2str(i),'}_{Xpar}.csv');
            writematrix([t(1) t(s.RecordLength) s.RecordLength],fnameX); 
